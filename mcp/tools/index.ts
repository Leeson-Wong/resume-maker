...
import type { ResumeData, Experience, Project, CareerSummary } from '../../src/types/resume.js';
...
export function registerTools(server: McpServer, resume: ResumeData): void {
  ...
  server.registerTool(
    'get_profile',
    {
      ...
      description:
        "Get the candidate's public profile: name, title, summary, and social links. Contact details (email, phone) are not exposed via MCP.",
    ...
    async ({ format }): Promise<CallToolResult> => {
      try {
        ...
        // Privacy: only expose name, title, summary (no email/phone/location)
